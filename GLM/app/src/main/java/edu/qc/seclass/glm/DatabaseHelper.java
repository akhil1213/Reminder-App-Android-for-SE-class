...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public String getAlarm(String desc, String type){
        SQLiteDatabase db = this.getWritableDatabase();
        String selectString = "SELECT * FROM " + REMINDER_TABLE + " WHERE " + COL2 + " = '" + desc + "' AND " + COL3 + " = '" + type + "'";
        Cursor cursor = db.rawQuery(selectString,null);
        cursor.moveToFirst();
        String hour = cursor.getString(4);
        String minute = cursor.getString(5);
        return hour + ":" + minute;
    }
